import { CaseStatusBadge } from "@/components/communication/case-status-badge";
...
import { CATEGORY_LABELS_SINGULAR, deriveCaseStatus, type EmailThreadWithRelations } from "@/lib/types";

export function ConversationRow({ thread }: { thread: EmailThreadWithRelations }) {
  const category = thread.category ?? "internal";
  const style = CATEGORY_STYLE[category];
  const Icon = style.icon;
  const status = deriveCaseStatus(thread);
  ...
  return (
    <div className="flex items-center gap-4 rounded-xl border border-[#eeeeee] bg-white px-4 py-3.5">
      ...
      <div className="w-32 shrink-0">
        <p className="text-[12.5px] text-[#9aa0a8]">{CATEGORY_LABELS_SINGULAR[category]}</p>
