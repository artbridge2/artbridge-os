...
import { addArtistDocument, removeArtistDocument } from "@/actions/artists";
import type { ArtistDocument } from "@/lib/types";

export function ArtistDocuments({ artistId, documents }: { artistId: string; documents: ArtistDocument[] }) {
  ...
  function add() {
    ...
    run(() => addArtistDocument(artistId, name, url));
    setName("");
    setUrl("");
    setAdding(false);
  }
  ...
  return (
    <div className="rounded-xl border border-[#eeeeee] bg-white p-4">
      <div className="flex items-center justify-between">
        <p className="text-[13px] font-semibold uppercase tracking-wide text-[#9aa0a8]">Documents</p>
        {!adding && (
      ...
      <div className="mt-2 flex flex-col gap-1.5">
        {documents.map((d) => (
          <div key={d.id} className="flex items-center gap-2 rounded-md border border-[#eeeeee] px-2.5 py-1.5">
            ...
            <button type="button" onClick={() => run(() => removeArtistDocument(d.id))} disabled={pending} className="text-[#9aa0a8] hover:text-[#e0353b]">
              <X className="size-3.5" />
            </button>
          </div>
        ))}
        {documents.length === 0 && !adding && <p className="text-[13px] text-[#9aa0a8]">No documents yet.</p>}
      </div>
      {adding && (
        <div className="mt-2 space-y-1.5">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Document name" className="h-8 w-full rounded-md border border-input bg-transparent px-2 text-[13px]" />
