...
/** "4 hours ago" / "2 days ago" / "1 week ago" style elapsed-time label, for the English Communication UI. */
export function formatElapsedEn(sinceIso: string): string {
  const minutes = Math.floor((Date.now() - new Date(sinceIso).getTime()) / 60_000);
